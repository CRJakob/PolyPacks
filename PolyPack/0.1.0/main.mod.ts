...
class PolyPackBase extends PolyMod {
    ...
    #setPackLoaded(pack: Pack | undefined, state: boolean) {
        if (!pack) return;
        pack.loaded = state;
        this.#savePacksToLocalStorage();
    }
    ...
    #reorderPack(pack: Pack | undefined, delta: number) {
        if (!pack) return;
    ...
    #openDescription = function (n, pack) {
    ...
    #createPackScreen = (n) => {
        ...
        let selectedPack: HTMLElement | null;
        ...
        unloadButton.addEventListener("click", () => {
            const pack = this.#getPack(selectedPack!.id.replace("pack:", ""))!;
            this.#setPackLoaded(pack, false);
            packsDiv.remove();
            this.#createPackScreen(n);
        });
        ...
        for (const pack of this.#getAllPacks()) {
            ...
            infoButton.addEventListener("click", () => {
                packsDiv.remove();
                n.playUIClick();
                this.#openDescription(n, pack);
            });
            packDiv.appendChild(infoButton);
            if (pack.loaded) activatedPacksContainer.appendChild(packDiv);
            else availableModsContainer.appendChild(packDiv);
        }
        ...
        removeButton.addEventListener("click", () => {
            n.playUIClick();
            this.#removePack(this.#getPack(selectedPack!.id.replace("pack:", "")));
            packsDiv.remove();
            this.#createPackScreen(n);
        });
        removeButton.disabled = true;
